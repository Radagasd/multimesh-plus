...
func _update_visual_instances_visibility() -> void:
	for data_group_idx in rid_data.size():
		var data_group : MMRidData = rid_data[data_group_idx]
...
func _add_visual_instance(group_idx : int, aabb : AABB) -> void:
	...
	rid_data[group_idx].multimesh_RID_map[aabb] = m_rid
	rid_data[group_idx].visual_instance_RID_map[aabb] = i_rid
...
func _remove_buffer(data_group_idx : int, aabb : AABB):
	var m_rid : RID = rid_data[data_group_idx].multimesh_RID_map[aabb]
	var i_rid : RID = rid_data[data_group_idx].visual_instance_RID_map[aabb]
	RenderingServer.free_rid(i_rid)
	RenderingServer.free_rid(m_rid)
	rid_data[data_group_idx].visual_instance_RID_map.erase(aabb)
	rid_data[data_group_idx].multimesh_RID_map.erase(aabb)
...
func _update_buffer(data_group_idx : int, buffer_map : Dictionary[AABB, PackedFloat32Array]) -> void:
	for aabb in buffer_map:
		...
		if !rid_data[data_group_idx].multimesh_RID_map.has(aabb):
...
func flush() -> void:
	for data_group_idx in rid_data.size():
		for aabb in rid_data[data_group_idx].visual_instance_RID_map:
			RenderingServer.free_rid(rid_data[data_group_idx].visual_instance_RID_map[aabb])
		rid_data[data_group_idx].visual_instance_RID_map = {}

		for aabb in rid_data[data_group_idx].multimesh_RID_map:
			RenderingServer.free_rid(rid_data[data_group_idx].multimesh_RID_map[aabb])
		rid_data[data_group_idx].multimesh_RID_map = {}
